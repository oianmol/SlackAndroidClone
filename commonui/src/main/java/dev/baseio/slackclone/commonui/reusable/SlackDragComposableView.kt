...
private fun InitialOffsetsSideEffect(
  ...
  SideEffect {
    coroutineScope.launch {
      sideNavOffX.animateTo(viewOffset(isLeftNavOpen, mainScreenOffset))
    }
    coroutineScope.launch {
      chatViewOffX.animateTo(viewOffset(isChatViewClosed, chatScreenOffset))
    }
  }
}
...
private fun mainScreenModifier(
  ...
  .pointerInput(Unit) {
    detectHorizontalDragGestures({
      ...
      //end
      sideNavigationEndTransition(offsetX, dragOffset, coroutineScope, onOpenSideNav)
      rightViewEndTransition(chatViewOffX,chatScreenOffset,coroutineScope,onOpenCloseRightView)
    }, {
      //cancel
